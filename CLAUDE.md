...
- `app/services/` — shared write/service layer used by both `app/web/` and `app/api/`. `recipes.py`: `RecipeDraft` (Pydantic, JSON-native intermediate form), `build_markdown` (assembles canonical file text via ruamel `CommentedMap`), `find_recipe_file` (tree-wide slug→path resolver), `slug_in_use` (global), `resolve_new_recipe_path` (validates the optional new-recipe folder), `_write_and_sync` (write + `sync_one`, restores original on failure), and `create_recipe`/`update_recipe`/`set_archived`/`set_favorite` returning `WriteOutcome` (`.ok`, `.issues`, `.sync_errors`) or raising `RecipeNotFoundError`. Neither raises `HTTPException` — callers (web routes, API endpoints) translate outcomes/exceptions into their own response shapes.
- `app/web/` — FastAPI router + Jinja templates + Markdown filter.
  - `library.py` (`GET /` full page, `GET /search` HTMX fragment with OOB facets), `recipe.py` (`GET /r/{slug}`), `deps.py` (cached `Settings` / `db_path` / `Jinja2Templates` providers — tests override `get_db_path`, `get_recipes_dir`, `get_users_path`, `get_tokens_path`), `markdown.py` (singleton `MarkdownIt` + `md` Jinja filter), `__init__.py` (combined router).
  - `forms.py` — `FormData` dataclass, `parse_form` (decodes `request.form()`), `form_to_draft` (converts `FormData` → `RecipeDraft`, parsing the ingredients-YAML textarea and comma-separated fields). Re-exports `RecipeDraft`/`build_markdown`/`find_recipe_file`/`resolve_new_recipe_path`/`slug_in_use` from `app.services.recipes` for existing imports.
  - `crud.py` — `GET/POST /new`, `GET/POST /r/{slug}/edit`, `POST /r/{slug}/archive|unarchive|favorite|unfavorite`. Routes call `app.services.recipes` functions and translate `WriteOutcome`/`RecipeNotFoundError` into HTML re-renders or `HTTPException`. Every write route depends on `require_user` (see auth conventions below).
  - `auth.py` — `GET/POST /login`, `POST /logout`. Verifies against `app.auth.store`, sets `request.session["user"]`.
- `app/api/` — JSON API, `prefix="/api/v1"`, mounted in `app/main.py`. `recipes.py` (`GET /recipes`, `GET /facets`, `GET /recipes/{slug}` — public, reuse `app/db/queries.py`; `POST /recipes`, `PUT /recipes/{slug}`, `PUT /recipes/{slug}/archived`, `PUT /recipes/{slug}/favorite` — Bearer-token-gated, call `app.services.recipes`), `schemas.py` (Pydantic request/response models; `RecipeWriteRequest.to_draft()` → `RecipeDraft`), `deps.py` (`require_token`), `__init__.py` (router).
...
- `app/auth/` — login auth. `store.py` (`load_users`/`set_password`/`delete_user`/`verify`; argon2 hashes in `DATA_DIR/auth.json`, atomic writes). `tokens.py` (`load_tokens`/`create_token`/`revoke_token`/`verify_token`; SHA-256 hashes in `DATA_DIR/api_tokens.json`, atomic writes). Both stores live **outside** the rebuildable `recipes.db` so they survive `recipes rebuild-index`. Login/logout routes are in `app/web/auth.py`.
- `app/cli.py` — operator surface (Typer): `validate`, `sync`, `rebuild-index`, `search`, `show`, `doctor`, `save-recipe`, `set-password`, `list-users`, `delete-user`, `create-token`, `list-tokens`, `revoke-token`, `run-dev`.
- `app/config.py` — env-driven paths (`RECIPES_DIR`, `DATA_DIR`) + auth settings (`auth_path`, `tokens_path`, `session_secret` from `SESSION_SECRET` or a persisted `.session_secret`, `cookie_secure` from `COOKIE_SECURE`).
...
- `tests/` — `test_parser_roundtrip.py`, `test_validator.py`, `test_sync_idempotent.py`, `test_fts_search.py`, `test_healthz.py`, `test_importer_save.py`, `test_db_queries_library.py`, `test_web_library.py`, `test_web_recipe.py`, `test_web_crud.py`, `test_auth.py`, `test_auth_tokens.py`, `test_api_read.py`, `test_api_write.py`. `conftest.py` provides `recipes_dir` (→ `tests/fixtures/recipes/`), `tmp_db`, `populated_db`, `client` (read-only; `get_db_path` overridden), `auth_users_path` (temp `auth.json` seeded with `TEST_USERNAME`/`TEST_PASSWORD`), `crud_client` (CRUD; `get_db_path`+`get_recipes_dir`+`get_users_path` overridden, **and logged in** so existing write tests pass), `anon_client` (same overrides, not logged in — for the auth gate), `api_tokens_path` (temp `api_tokens.json` seeded with one token, returns `(path, plaintext)`), and `api_client` (CRUD-style overrides + `get_tokens_path`, **no session login**, returns `(client, plaintext_token)`). conftest sets `COOKIE_SECURE=false`/`SESSION_SECRET` before importing the app so the test client's http cookie works.
...
## CRUD write conventions (Stage 4 + M3)

- All writes flow through `app/services/recipes.py`: `build_markdown` → `_write_and_sync` → `sync_one`. Never write to SQLite without first updating the Markdown file. This module is shared by `app/web/crud.py` and `app/api/recipes.py` — there is exactly one write path.
- `build_markdown` takes a `RecipeDraft` (JSON-native: `int | None` minutes, `list[str]` tags/etc., `ingredients: list[dict[str, Any]]`, `body: str`) and constructs a ruamel `CommentedMap` with the same `_yaml()` settings as `serializer.py` (same instance; import `_yaml` from there). This ensures the generated file is roundtrip-stable immediately — no second-pass reformat needed. The web layer builds a `RecipeDraft` from form input via `app/web/forms.py::form_to_draft`; the API builds one via `RecipeWriteRequest.to_draft()`.
...
- Slug is derived from title via `normalize_slug()` on create; immutable on edit (the route path owns it). Collision check via `slug_in_use(recipes_dir, slug)` (tree-wide) before writing. New recipes may target a subdirectory via `RecipeDraft.folder`, validated by `resolve_new_recipe_path` (rejects absolute/`..`/reserved-dir paths, traversal-guarded). Edits and bool-field flips resolve the existing file with `find_recipe_file` and rewrite it in place, preserving its subdirectory.
- `create_recipe`/`update_recipe`/`set_archived`/`set_favorite` (in `app/services/recipes.py`) return a `WriteOutcome` (`slug`, `path`, `issues`, `sync_errors`, `.ok`) for validation/sync failures, or raise `RecipeNotFoundError` when `update`/`set_*` target a missing slug. Callers translate these: web routes re-render the form with errors or raise `HTTPException`; API endpoints map to 404/409/422/500 JSON envelopes (see Bearer auth conventions below).
- Archive/unarchive and favorite/unfavorite share `_flip_bool_field`, which mutates `doc.raw_yaml[field]` and `doc.raw_yaml["updated_at"]` in-place, then calls `serialize(doc)`. This is the one place we ruamel-edit rather than rebuild — it preserves all other YAML formatting. The web favorite/unfavorite routes accept a `?next=` query param (same-origin only, via `_safe_next`) so the card heart can return to the library.
...
- Session is a signed cookie (`SessionMiddleware` in `app/main.py`, built from `load_settings()` at import). `cookie_secure` (env `COOKIE_SECURE`, default true) sets the `Secure` flag — TLS is terminated by a reverse proxy, not the app. Set `SESSION_SECRET` in production. Note: `SessionMiddleware` has no `http_only` kwarg (always `HttpOnly`) — don't add one, it raises `TypeError`.
...
- **CSRF is handled by `SameSite=Lax` + POST-only mutations, not tokens** (Lax withholds the cookie on all cross-site subrequests; GET routes never mutate). Don't add CSRF token machinery unless untrusted content ingestion or multi-tenant use lands. Rationale in `docs/architecture.md` ("Why no CSRF tokens").
- Hardening invariants to preserve: login calls `request.session.clear()` before setting the user; `store.verify` always runs argon2 (dummy hash for unknown users) to avoid timing enumeration; the persisted `.session_secret` is written `0o600`; the `next` param is `_safe_next`-validated and URL-encoded into the redirect.

## API auth conventions (Stage M3)

- **`app/api/` reads are public** (same as web); **writes require a Bearer token**, independent of the web session cookie. `app/api/deps.py::require_token` (an `HTTPBearer(auto_error=False)` dependency) verifies the `Authorization: Bearer <token>` header against `app/auth/tokens.py::verify_token`; missing/invalid tokens get a 401 with a `WWW-Authenticate: Bearer` header. A logged-in browser session does **not** grant API write access — `tests/test_api_write.py` asserts this with the `crud_client` fixture.
- Tokens are `recipes_` + `secrets.token_urlsafe(32)`, stored as SHA-256 hashes (not argon2 — see `docs/architecture.md` "Why the API uses bearer tokens") in `DATA_DIR/api_tokens.json` (`settings.tokens_path`), a sibling of `auth.json` outside the rebuildable DB. Manage via `recipes create-token|list-tokens|revoke-token`; `create-token` prints the plaintext once.
- API error envelopes: validation/sync failures from `WriteOutcome` become `HTTPException(status, detail={"code", "message", "issues": [...]})` — 409 for `slug.collision`, 422 for other validation issues, 500 for `sync_errors`. `RecipeNotFoundError` → 404. FastAPI's native 422 (request-body schema errors) is left as-is.
