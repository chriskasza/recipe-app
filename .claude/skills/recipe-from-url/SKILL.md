...
The intelligent half of `recipes import-url <url>` (TODO.md Stage 5) — fetching the page and extracting fields — is still done here by the agent; the mechanical write half now ships in the app as `recipes build-draft`. The draft lives under `_drafts/` so the user can review and edit before promoting it to the canonical `recipes/` directory.
...
```bash
docker compose exec -T app recipes build-draft --json < tmp/recipe-payload.json
```

`-T` disables the TTY so the stdin redirect works. The payload stays on the host; the shell feeds it in over stdin. The command writes the draft to `/app/recipes/_drafts/` inside the container, which is the bind-mounted `./recipes/` volume, so the file appears on the host immediately. No `recipes sync` is needed — `_drafts/` is a review staging area, not part of the synced library.

(If the `recipes` entry point isn't on PATH in the image, fall back to `docker compose exec -T app python -m app.cli build-draft --json`.)
...
- **Multiple recipes on one page** (e.g. "3 variations on miso eggplant"): the schema is one file per recipe. Ask the user which variation to import, or whether to make multiple drafts (one URL → multiple `build-draft` runs with distinct slugs).
